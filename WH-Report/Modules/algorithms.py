...
def generate_wh_report(df, df_event, date_vals, site_code, de=pd.Timedelta("30 minutes")):
  '''
...
##################################################
    ...
    projected_dg_hrs = (dg_ign_time - dg_time)/3600 if (dg_ign_time - dg_time)/3600 > 0.2 else 0
    ...
    if missing_hrs == 0:
      ...
      output_df[STRUCT[2]].append(
        23.99-missing_hrs-(dg_time/3600)-(ac_100_time/3600))
    ...
    if missing_hrs == 0:
      output_df[STRUCT[24]].append(dg_wh + projected_dg_wh + projected_eb_wh + ebwh)
    else:
