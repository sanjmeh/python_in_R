...
def internal_agg(input_df,input_mods_df,dist,n,termid): 

    df,mods_df = data_prep_distance(input_df,input_mods_df)
    list_=[]
    max_ = round_to_nearest(df['cumsum_dist_Sir'].max(),n)
    for i in range(dist,max_,dist):
        ...
        if len(sample)!=0:
            ...
            temp_dict['Class'] = termid_class_map[str(termid)]
            temp_dict['total_samples'] = len(sample)
            temp_dict['start_time'] = sample.head(1)['ts'].item()
            temp_dict['end_time'] = sample.tail(1)['ts'].item()
            temp_dict['Initial_level'] = sample.head(1)['fuel'].item()
            temp_dict['End_level'] = sample.tail(1)['fuel'].item()
            temp_dict['Refuel_sum'] = sample['REfuel_unique'].sum()
            total_time = sample['time_diff'].sum();ig_time = sample.query("currentIgn==1")['time_diff'].sum()
            temp_dict['total_dist'] = sample['disthav'].sum()
...
def distance_algo(input_df,input_mods_df,termid):
    ...
    if termid_class_map[str(termid)] == 'high_movement':
        ...
        list_1 = internal_agg(df,mods_df,100,100,termid)
